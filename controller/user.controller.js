...
const checkSq = async (req, res) => {
    ...
    try {
        ...
        jwt.verify(token, "secret", async (err, decoded) => {
            ...
            console.log(decoded);
            const particularUser = await pool.query("SELECT * FROM users WHERE uid=($1)", [decoded.id])
            if (particularUser) {
                if (sq === "200")
                    res.json({
                        reqSq: sq,
...
module.exports = { createUser, getAllUser, getUserById, updateUser, deleteUser, signIn, checkSq }